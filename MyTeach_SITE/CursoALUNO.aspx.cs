...
public partial class CursoALUNO : System.Web.UI.Page
{
    ...
    {
        //criptografar a info que ele pesquisou

        //criando variavel para tabela
        DataView ListaCat;
        ListaCat = (DataView)sqlListaCategoria.Select(DataSourceSelectArguments.Empty);

        /*impando ddl para o ddl não ficar adicionando mais informações iguais*/
        ListBox1.Items.Clear();


        //varrer a lista de produtos e add ao ddl já descriptografado(fazendo ddl a mão)
        //laço for que repete oq tem nas chave(valor inicial que começa com zero ; / valor de parada; adiciona um para cada rodada)

        for (int i = 0; i < ListaCat.Table.Rows.Count; i++)
        {
            //Adicionar os itens ao ddl
            ListBox1.Items.Add(new ListItem(cripto.Decrypt(ListaCat.Table.Rows[i]["nome_cat"].ToString()), ListaCat.Table.Rows[i]["id_cat"].ToString()));
